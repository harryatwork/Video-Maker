...
// $sql_slide_preview_slides = "SELECT * FROM video_maker_scene WHERE p_id = '$p_id'";
// $result_slide_preview_slides = $conn->query($sql_slide_preview_slides);
// if ($result_slide_preview_slides->num_rows > 0) {   
// while($row_slide_preview_slides = $result_slide_preview_slides->fetch_assoc()) { 
//     $scene_id = $row_slide_preview_slides["id"];
...
$("#overlay2,.slide_preview_overlay").on("click",function() {
    ...
    $(".video_preview_content_div").html("");
    $(".video_preview_content_div").css("display","none");
    $(".video_preview_overlay").css("display","none");
    $(".video_preview_overlay").css("background","#0000007a");
    $(".video_preview_loading_div").css("display","block");
    $(".video_preview_overlay_another").css("display","none");
    slide_preview_reset();
    clearTimeout(interval);
});

$(".video_preview_overlay,.video_preview_overlay_another").on("click",function() {
    if(slide_preview_state == 'playing') {
        ...
        $(".video_preview_overlay").css("background","transparent");
        $('.video_preview_play').fadeOut({queue: false, duration: 'fast'});
        $('.video_preview_play').animate({'zoom': 2}, 300);
...
$(".video_preview_repeat").on("click",function() {
    $(".video_preview_overlay").css("background","transparent");
    $('.video_preview_repeat').fadeOut({queue: false, duration: 'fast'});
    $('.video_preview_repeat').animate({'zoom': 2}, 300);
    $(".video_preview_play").css("display","block");
    $('.video_preview_play').css("left","50%");
    slide_preview_state = 'paused';
});
...
function slide_preview_reset() {
    $(".video_preview_overlay").css("background","#0000007a");
    $('.video_preview_play').css("left","50%");
    $('.video_preview_play').fadeIn({queue: false, duration: 'fast'});
    $('.video_preview_play').animate({'zoom': 1}, 300);
    $(".video_preview_repeat").css("display","block");
    $('.video_preview_repeat').css("display","none");
    slide_preview_state = 'paused';
    $(".video_preview_layer").css("display","none");
    // clearTimeout(interval);
	if($('#bgm_palyer').length) {
		$('.bgm_palyer').get(0).pause();
