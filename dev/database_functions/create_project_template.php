<?php include("../db.php");
    ...
    $sql_project = "SELECT * FROM video_maker WHERE id = '$p_id'";
    $result_project = $conn->query($sql_project);
    if ($result_project->num_rows > 0) {   
    while($row_project = $result_project->fetch_assoc()) {
        $p_title = $row_project["title"];
        $p_uid = $row_project["u_id"];

        $sql_project_insert = "INSERT INTO video_maker_templates (p_id, u_id, title, date)
                               VALUES ('$p_id', '$p_uid', '$p_title', '$date')";
        if ($con->query($sql_project_insert) === TRUE) { 
            $project_template_id = $con->insert_id;

                $sql_scene = "SELECT * FROM video_maker_scene WHERE p_id = '$p_id'";
                ...
                while($row_scene = $result_scene->fetch_assoc()) {
                    ...
                    $sql_scene_insert = "INSERT INTO video_maker_scene_templates (p_id, u_id, title, background, duration, date)
                            VALUES ('$project_template_id', '$u_id', '$scene_title', '$scene_background', '$scene_duration', '$date')";
            ...
            //Layers ----------------------------------------------------------
                        ...
                        while($row_layers = $result_layers->fetch_assoc()) {
                            ...
                            $sql_layer_insert = "INSERT INTO video_maker_layers_templates (title, p_id, u_id, scene_id, layer, layer_id, layer_count, content, date)
                                                VALUES ('$layer_title', '$project_template_id', '$u_id', '$scene_template_id', '$layer_layer', '$layer_layer_id', '$layer_layer_count', '$layer_content', '$date')";
            ...
            // Draggable Layers ------------------------------------------------
                                ...
                                while($row_draggable_layers = $result_draggable_layers->fetch_assoc()) {
                                    ...
                                    $sql_draggable_layer_insert = "INSERT INTO video_maker_draggable_layers_templates (main_layer_id, p_id, u_id, layer, title, layer_count, layer_length, start_time_sec, end_time_sec, date)
                                                VALUES ('$layer_template_id', '$project_template_id', '$u_id', '$layer_draggable_layer', '$layer_draggable_title', '$layer_draggable_layer_count', '$layer_draggable_layer_length', '$layer_draggable_start_time_sec', '$layer_draggable_end_time_sec', '$date')";
